...
df = pd.concat([original_df['session ID'],
               original_df['main category'].astype(str) + '-' + original_df['colour'].astype(str)], axis=1)
df.columns = ['session_ID', 'clothing']
# print(df)
# quit()

df = df.groupby("session_ID")['clothing'].apply(
...
df['Transaction_list'] = df['clothing'].apply(transaction_format)
...
print("Regras")
rules = association_rules(frequent_itemsets, metric='confidence', min_threshold=0.9, support_only=False).sort_values(
...
#               & (rules['lift'] >= 1)]
# print(rules.loc[rules['antecedents'] == '(1-3)'])
