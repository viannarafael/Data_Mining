...
model_params = {
    ...
    'SVC': {
        'model': LinearSVC(max_iter=100000, tol=1e-4),
        'params': {'C': [0.5, 1, 5, 10, 15, 20],
    ...
    'MLP': {
        ...
        'params': {'hidden_layer_sizes': [30, 50, 100, 200, 500], 'activation': ['relu', 'logistic', 'tanh'], 'solver': ['lbfgs', 'sgd', 'adam'], 'learning_rate': ['constant', 'invscaling', 'adaptive']}
    }
}
...
for model_name, mp in model_params.items():
    ...
    clf = RandomizedSearchCV(
        mp['model'], mp['params'], cv=20, n_iter=10, n_jobs=-1)
    clf.fit(X, Y)
